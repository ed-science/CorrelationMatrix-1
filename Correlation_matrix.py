    ...
    for i in range(ds[0]):
        for j in range(ds[0]):
            if i == j:
                ...
                ax[i,j].set_yticks([])
                if j == ds[0]-1:
                    ax[i,j].set_ylabel(f"var_{i + 1}", fontsize=11).set_color("red")
                    ax[i,j].yaxis.set_label_position("right")

                if i == 0 and j == 0:
                    ax[i,j].set_title(f"var_{i + 1}", fontsize=11).set_color("red")
                ...
                if prs >= 0.5 or prs <= -0.5:
                    ...
                    ax[i, j].text(
                        0.5,
                        0.5,
                        str(pearsonr(data[i], data[j])[0])[:4],
                        fontsize=10,
                        horizontalalignment='center',
                        verticalalignment='center',
                    )
                ...
                if i ==0:
                    ax[i,j].set_title(f"var_{j + 1}", fontsize=11).set_color("red")
                    ax[i,j].set_xticks([])
                    ax[i,j].set_yticks([])
                if j == ds[0]-1:
                    ax[i,j].set_ylabel(f"var_{i + 1}", fontsize=11).set_color("red")
